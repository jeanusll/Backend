...
def create_solicitud():
    try:
        content = solicitud_model.create_solicitud(request.json['id_asistencia'], request.json['documento'], request.json['estado'], request.json['justificacion'])    
...
def update_solicitud():
    try:
        content = solicitud_model.update_solicitud(request.json['id_solicitud'], request.json['id_asistencia'], request.json['documento'], request.json['estado'], request.json['justificacion'])    
        return jsonify(content)
    except KeyError as e:
        return jsonify({'Error': f'Falta el campo requerido: {str(e)}'}), 400
    except Exception as e:
