...
class AlumnoModel:
    ...
    def get_alumnos(self):  
        rv = self.mysql_pool.execute("""SELECT * from alumnos a inner join usuarios on a.id_usuario = usuarios.id_usuario""")  
        data = []
        content = {}
        for result in rv:
            content = {'dni_alumno': result[0],'id_usuario': result[1], 'nombre': result[2], 'apellido': result[3], 'fecha_nacimiento':result[4], 'foto':result[5], 'vector':result[6], 'email' : result[9], 'contra' : result[10] }
            data.append(content)
            content = {}
        return data
    ...
    def create_alumno(self, dni_alumno, id_usuario, nombre, apellido, fecha_nacimiento, foto):
        ...
        response = requests.post(" http://127.0.0.1:81/openfaceAPI", files = {"file" : open("{}".format(foto), "rb")})
