...
class UsuarioModel:
    ...
    def iniciar_sesion(self, email, contra):    
        params = {'email' : email}      
        rv = self.mysql_pool.execute("""SELECT * from usuarios where email = %(email)s""", params)                
        data = []
        content = {}
        for result in rv:
            content = {'id_usuario': result[0], 'id_grupo_usuario': result[1], 'email': result[2],'contra': result[3]}
            data.append(content)
            content = {}
        if len(data) > 0:

            if self.verificar_password(contra, data[0]["contra"].encode('utf-8')):
                return data
            else:
